...
var import_axios = __toESM(__nccwpck_require__(8757));
function run() {
  return __async(this, null, function* () {
    try {
      const webhook = process.env.WECHAT_WORK_BOT_WEBHOOK || core.getInput("webhook");
      const msgtype = core.getInput("msgtype") || "text";
      ...
      const mentioned_list_obj = (mentioned_list == null ? void 0 : mentioned_list.length) > 0 ? { mentioned_list } : {};
      const mentioned_mobile_list_obj = (mentioned_mobile_list == null ? void 0 : mentioned_mobile_list.length) > 0 ? { mentioned_mobile_list } : {};
      if (msgtype === "text") {
        const payload = {
          msgtype,
          text: __spreadValues(__spreadValues({
            content
          }, mentioned_list_obj), mentioned_mobile_list_obj)
        };
        console.log("[wechatwork-action] Payload data:", JSON.stringify(payload));
        console.log("[wechatwork-action] Sending text message ...");
        const { data } = yield import_axios.default.post(webhook, JSON.stringify(payload), {
          headers: {
            "Content-Type": "application/json"
          }
        });
      ...
      if (msgtype === "markdown") {
        const payload = {
          msgtype,
          markdown: __spreadValues(__spreadValues({
            content
          }, mentioned_list_obj), mentioned_mobile_list_obj)
        };
        console.log("[wechatwork-action] Payload data:", JSON.stringify(payload));
